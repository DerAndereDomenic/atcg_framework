...
coopVecMatMul(const OptixCoopVec<half, input_size>& input, CUdeviceptr weights, size_t weights_offset)
{
    using T_INPUT  = OptixCoopVec<half, input_size>;
    using T_OUTPUT = OptixCoopVec<half, output_size>;

    T_OUTPUT output =
        optixCoopVecMatMul<T_OUTPUT,
                           T_INPUT,
                           OPTIX_COOP_VEC_ELEM_TYPE_FLOAT16,
                           layout,
                           transpose,
                           output_size,
                           input_size,
                           OPTIX_COOP_VEC_ELEM_TYPE_FLOAT16>(input, weights, weights_offset, sizeof(half) * input_size);
    return output;
}
