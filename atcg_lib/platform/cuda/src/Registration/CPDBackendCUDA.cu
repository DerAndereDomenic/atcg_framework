...
namespace detail
{
__device__ inline uint32_t globalThreadIndex()
{
    return blockIdx.x * blockDim.x + threadIdx.x;
}

__host__ __device__ inline thrust::pair<uint32_t, uint32_t> index2pixel(const uint32_t& index, const uint32_t& width)
{
...
__host__ inline KernelSize configure(const uint32_t& size)
{
    KernelSize result;
    ...
    if(size % result.THREADS_PER_BLOCK == 0) { ++result.blocks; }
...
struct Pmn
{
    ...
    __device__ inline double operator()(uint32_t m)
    {
        double* x = X + 3 * n;
        double* y = Y + 3 * m;
...
__global__ void
fillP(double* X, double* Y, double* P, double* R, double* t, double* Z, double s, double var, uint32_t N, uint32_t M)
{
...
CPDBackendCUDA::CPDBackendCUDA(RowMatrix& X, RowMatrix& Y) : CPDBackend(X, Y)
{
...
{
    cudaSafeCall(
        cudaMemcpy((void*)(impl->devR), (void*)&transform.R(0), sizeof(double) * 3 * 3, cudaMemcpyHostToDevice));
    ...
    cudaSafeCall(cudaMemcpy((void*)&impl->P(0),
                            (void*)(impl->devP),
                            sizeof(double) * impl->N * impl->M,
    ...
    for(uint32_t n = 0; n < impl->N; ++n)
    {
        ...
        Z[n] = thrust::transform_reduce(thrust::device,
                                        ...
                                        detail::Pmn(impl->devX, impl->devY, impl->devR, impl->devT, transform.s, var,
    n), 0, thrust::plus()); stats.addSample(t.elapsedMillis());
    }
    std::cout << stats;*/
    // for(m -> M)
    // double result = thrust::reduce(impl->devP, impl->devP + impl->N*impl->M, 0, thrust::plus());
    // int m = 0;
    // thrust::transform_reduce(thrust::device, thrust::counting_iterator<int>(0),
    // thrust::constant_iterator<int>(impl->N), [&](int n){return impl->devP[m + n*impl->N];}, 0, thrust::plus());
}

void CPDBackendCUDA::maximize(const RowMatrix& XC, const RowMatrix& YC, RowMatrix& A)
{
