...
extern "C" ATCG_RT_EXPORT void __closesthit__mesh()
{
    ...
    glm::vec3 ray_origin = getWorldRayOrigin();
    glm::vec3 ray_dir    = getWorldRayDirection();
    float tmax           = getRayTmax();

    si->valid              = true;
    si->incoming_distance  = tmax;
    si->incoming_direction = ray_dir;
    si->primitive_idx      = getPrimitiveIndex();
    si->barys              = getTriangleBarycentrics();
    ...
    glm::vec3 optix_pos = transformPointFromObjectToWorldSpace(si->position);
    ...
    // Transform local position to world position
    si->normal = transformNormalFromObjectToWorldSpace(si->normal);
