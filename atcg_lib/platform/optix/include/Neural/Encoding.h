...
#include <Core/CUDA.h>
#include <Neural/DeviceHashGrid.h>    // Also an encoding

namespace atcg
{
ATCG_INLINE ATCG_HOST_DEVICE void oneBlob(float position, uint32_t k, float* encoding)
{
    float sigma = 1.0f / static_cast<float>(k);
    // Sample in the middle of a bin.
    float current_pos = sigma / 2.0f;
    for(uint32_t i = 0; i < k; ++i)
    {
        encoding[i] = glm::exp(-0.5f * (position - current_pos) * (position - current_pos) / (sigma * sigma)) /
                      (glm::sqrt(2.0f * glm::pi<float>()) * sigma);

        current_pos += sigma;
    }
}

ATCG_INLINE ATCG_HOST_DEVICE void oneBlobPeriodic(float position, uint32_t k, float* encoding)
{
    float sigma = 1.0f / static_cast<float>(k);
    // Sample in the middle of a bin.
    float current_pos = sigma / 2.0f;
    for(uint32_t i = 0; i < k; ++i)
    {
        float d     = glm::abs(position - current_pos);
        d           = d > 0.5f ? 1.0f - d : d;
        encoding[i] = glm::exp(-0.5f * d * d / (sigma * sigma)) / (glm::sqrt(2.0f * glm::pi<float>()) * sigma);

        current_pos += sigma;
    }
}

ATCG_INLINE ATCG_HOST_DEVICE void frequencyEncoding(float position, uint32_t k, float* encoding)
{
    float frequency = 1.0f;
    for(uint32_t d = 0; d < k; ++d)
    {
        encoding[d] = glm::sin(frequency * glm::pi<float>() * position);
        frequency *= 2.0f;
    }
}

template<int degree>
ATCG_INLINE ATCG_HOST_DEVICE void sphericalHarmonicEncoding(float x, float y, float z, float* data_out)
{
    ...
    // SH polynomials generated using scripts/gen_sh.py based on the recurrence relations in appendix A1 of
    ...
    data_out[22] =
        (0.47308734787878004f * (x2 - y2) * (7.0f * z2 - 1.0f));      // 3*sqrt(5)*(x2 - y2)*(7*z2 - 1)/(8*sqrt(pi))
    ...
    data_out[25] = (0.65638205684017015f * y *
    ...
    data_out[30] = (0.1169503224534236f * z *
    ...
    data_out[35] = (0.65638205684017015f * x *
    ...
    data_out[37] = (2.3666191622317521f * yz *
    ...
    data_out[40] = (0.92120525951492349f * xy *
                    (-18.0f * z2 + 33.0f * z4 + 1.0f));    // sqrt(2730)*xy*(-18*z2 + 33*z4 + 1)/(32*sqrt(pi))
    data_out[41] = (0.58262136251873131f * yz *
    ...
    data_out[43] = (0.58262136251873131f * xz *
    ...
    data_out[46] = (0.50456490072872406f * (11.0f * z2 - 1.0f) *
                    (-6.0f * x2 * y2 + x4 + y4));    // 3*sqrt(91)*(11*z2 - 1)*(-6*x2*y2 + x4 + y4)/(32*sqrt(pi))
    data_out[47] = (2.3666191622317521f * xz *
                    (10.0f * x2 * y2 - x4 - 5.0f * y4));    // 3*sqrt(2002)*xz*(10*x2*y2 - x4 - 5*y4)/(32*sqrt(pi))
    data_out[48] = (10.247761577878714f * x2 * y4 - 10.247761577878714f * x4 * y2 + 0.6831841051919143f * x6 -
    ...
    data_out[60] = (1.0378311574405206f * z * (13.0f * z2 - 3.0f) *
