               ...
               torch::PackedTensorAccessor32<uint8_t, 3, torch::RestrictPtrTraits> output_image)
{
    ...
    for(auto tid = id; tid < width * height; tid += num_threads)
    {
        ...
        glm::vec3 tone_mapped = glm::vec3(1.0f) - glm::exp(-radiance);
        glm::vec3 sRGB        = atcg::Color::lRGB_to_sRGB(tone_mapped);
...
void PipelineInitializer<HDRFilm>::apply(const atcg::ref_ptr<HDRFilm>& component) const
{
