...
namespace atcg
{
    NonRigidCoherentPointDrift::NonRigidCoherentPointDrift(const std::shared_ptr<PointCloud>& source, const std::shared_ptr<PointCloud>& target, const double& w)
    ...
    double NonRigidCoherentPointDrift::maximize(Eigen::VectorXd& PX, Eigen::VectorXd& PY, double var)
    {
        ...
        var = Np/3 * ((X.transpose() * PX.asDiagonal() * X).trace() - 2.0 * ((P*X).transpose()*T).trace() + (T.transpose()*PY.asDiagonal()*T).trace());
        return var;
    }
