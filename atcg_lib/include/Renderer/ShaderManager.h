...
namespace atcg
{
    ...
    public:
         ...
         */
        inline static void addShader(const std::string& name, const std::shared_ptr<Shader>& shader) {s_instance->addShaderImpl(name, shader);}
         ...
         */
        inline static const std::shared_ptr<Shader>& getShader(const std::string& name) {return s_instance->getShaderImpl(name);}
