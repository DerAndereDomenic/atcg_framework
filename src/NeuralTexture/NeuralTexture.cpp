...
NeuralTexture::NeuralTexture(const atcg::ref_ptr<atcg::RaytracingContext>& context)
{
    ...
    _weights = torch::randn({8 * 64 + 3 * 64 * 64 + 8 * 64}, atcg::TensorOptions::floatDeviceOptions()) * 0.1f;
    _weights = _weights.requires_grad_(true);
    _bias    = torch::randn({64 + 64 + 64 + 64 + 8}, atcg::TensorOptions::floatDeviceOptions()) * 0.1f;
    _bias    = _bias.requires_grad_(true);

    _mlp = atcg::MLP<3, 8, 64, 8>(context, _weights.to(torch::kFloat16), _bias.to(torch::kFloat16));
}
