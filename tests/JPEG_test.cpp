...
TEST(JPEGDecoderTest, decode)
{
    ...
    // TODO: Image needs to be flipped
...
TEST(JPEGDecoderTest, decodeFlip)
{
    ...
    atcg::JPEGDecoder decoder(1, target->width(), target->height(), true);
...
TEST(JPEGDecoderTest, batchDecodeFlip)
{
    ...
    atcg::JPEGDecoder decoder(batch_size, target->width(), target->height(), true);
